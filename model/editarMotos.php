...
if((!isset($_FILES["fot"]["type"]))){
    ...
    if ($continue == 0) {
        $sql = mysqli_query($mysqli, "UPDATE vehiculos SET cedula = '$Cedula', foto = foto, placa = '$Placa', modelo_cilindraje = '$Modelo', puertas_tiempos = '$Puertas', tipo = '2' WHERE id = '$id'");
        if($sql){
            $continue = 0;
        }else{
            $continue = 1;
    ...
    if ($continue == 0) {
        ...
        if (($_FILES["fot"]["type"] == "image/pjpeg")
            ...
            if (move_uploaded_file($_FILES["fot"]["tmp_name"], "../img/vehiculos/" . $imagen1 . ".png")) {
                ...
                $sql = mysqli_query($mysqli, "UPDATE vehiculos SET cedula = '$Cedula', foto = '$imagen1.png', placa = '$Placa', modelo_cilindraje = '$Modelo', puertas_tiempos = '$Puertas', tipo = '2' WHERE id = '$id'");
                if($sql){
                    $continue = 0;
                }else{
                    $continue = 1;
